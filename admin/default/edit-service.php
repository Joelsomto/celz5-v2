...
if (isset($_GET['ServiceId'])) {
    ...
    $service = $Controller->getServicesById($service_id);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $end_time = !empty($_POST['end_time']) ? $_POST['end_time'] : null; // Properly handle null
    $service_link = trim($_POST['service_link']);
    $backup_service_link = !empty($_POST['backup_service_link']) ? trim($_POST['backup_service_link']) : null;
    $status = trim($_POST['status']);
    $category_id = $_POST['category'];
    $service_id = $_POST['service_id'] ?? null; // Ensure service ID is provided

    // Validate inputs
    if (empty($title) || empty($description) || empty($start_time) || empty($service_link) || empty($category_id) || empty($service_id)) {
    ...
    function createSlug($string)
    {
        ...
        $string = preg_replace('/[\s-]+/', '-', $string); // Replace spaces and dashes with a single dash
    ...
    try {
        // Call the controller method to update the service
        $Controller->updateService($service_id, $title, $description, $category_id, $start_time, $end_time, $service_link, $backup_service_link, $status, $slug);

        // Success message
        $_SESSION['successMsg'] = 'Service updated successfully!';

        // Redirect to the services page or another page of your choice
        header('Location: edit-service.php?ServiceId=' . urlencode($service_id));
        exit();

    } catch (Exception $e) {
        // Handle any exceptions during the process
        $_SESSION['errorMsg'] = 'Error updating service: ' . $e->getMessage();
        header('Location: edit-service.php');
        exit();
    }
}
                ...
                <div class="row">
                    <div class="col-sm-12">
                        <div class="page-title-box">
                            <div class="row">
                                <div class="col">
                                    <h4 class="page-title">Edit (<?= $service[0]['title'] ?>)</h4>
                                    <ol class="breadcrumb">
                                        ...
                                        <li class="breadcrumb-item active">Edit (<?= $service[0]['title'] ?>)</li>
                                    </ol>
                                </div>
                                <!--end col-->
                    ...
                    <?php
                        ...
                        <div class="card">
                            <div class="card-header">
                                <h4 class="card-title">Edit (<?= $service[0]['title'] ?>)</h4>
                            ...
                            <div class="card-body">
                                ...
                                <input type="hidden" name="service_id" value="<?= $service[0]['service_id'] ?>" />
                                    <div class="form-group">
                                        <label>Title</label>
                                        <input type="text" value="<?= $service[0]['title'] ?>" name="title" class="form-control" required placeholder="" />
                                    </div>

                                    <div class="form-group">
                                        <label>Description</label>
                                        <textarea rows="5" name="description" class="form-control" required placeholder="Description"><?= htmlspecialchars($service[0]['description']) ?></textarea>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input name="service_link" value="<?= $service[0]['service_link'] ?>" type="text" class="form-control" required="" placeholder="URL" />
                                    </div>
                                    <!--end form-group-->
                                    <div class="form-group">
                                        <label>Backup Service Link  </label>
                                        <input type="text" value="<?= $service[0]['backup_service_link'] ?>" name="backup_service_link" class="form-control" placeholder="URL" />
                                    </div>
                                    <?php
                                        ...
                                        <select name="category" class="form-control" required placeholder="">
                                            ...
                                            <?php foreach ($getCategories as $category) { ?>
                                                <option value="<?= $category['cat_id'] ?>"
                                                    <?php if ($service[0]['category_id'] === $category['cat_id']) {
                                                        echo 'selected';
                                                    } ?>>
                                                    <?= $category['category'] ?>
                                                </option>
